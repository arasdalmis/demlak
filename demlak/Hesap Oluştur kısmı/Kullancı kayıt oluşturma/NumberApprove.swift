...
class NumberApprove: UIViewController, UITextFieldDelegate {
        ...
        if resendCount < 2 {
            ...
            if let phoneNumber = UserDefaults.standard.string(forKey: "userPhoneNumber") {
                ...
                PhoneAuthProvider.provider().verifyPhoneNumber(phoneNumber, uiDelegate: nil) { verificationID, error in
                    ...
                    // Konsola başarılı işlem mesajı yazdırabilirsiniz.
    ...
    @objc func ileriButtonTapped() {
        guard let verificationCode = verifyTextField.text, !verificationCode.isEmpty else {
            showAlert(message: "Doğrulama kodunu giriniz.")
            return
        }
        guard let verificationID = UserDefaults.standard.string(forKey: "authVerificationID") else {
            showAlert(message: "Doğrulama ID'si bulunamadı.")
            return
        }
        ...
        Auth.auth().signIn(with: credential) { [weak self] authResult, error in
            ...
            guard let phoneNumber = authResult?.user.phoneNumber else {
                self?.showAlert(message: "Telefon numarası alınamadı.")
                return
            }
            
            UserDefaults.standard.set(phoneNumber, forKey: "verifiedPhoneNumber")
            self?.navigateToCreatePassword()
            
            // Telefon numarası başarıyla doğrulandıktan sonra kullanıcı hesabını silme
            Auth.auth().currentUser?.delete { error in
                if let error = error {
                    // Hata yönetimi, kullanıcı silinirken bir hata oluşursa
                    print("Kullanıcı silinirken hata oluştu: \(error.localizedDescription)")
                } else {
                    // Kullanıcı başarıyla silindi, ilgili işlemler burada yapılabilir
       ...
       func navigateToCreatePassword() {
           DispatchQueue.main.async {
               let createPasswordVC = CreatePassword(nibName: "CreatePassword", bundle: nil)
